  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600">Manage cart orders and buy now requests</p>
            </div>
              ...
              <div className="flex rounded-lg border border-gray-200 p-1 bg-gray-50">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-1.5 ${
                    activeTab === 'buyNow' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                  }`}
                >
                  Buy Now ({requests.length})
                  {newRequestsCount > 0 && (
        ...
        {activeTab === 'buyNow' ? (
          /* Buy Now Requests Tab */
          <>
            {/* Filters */}
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap gap-2">
                {['all', ...REQUEST_STATUS_OPTIONS.map((s) => s.value)].map((status) => (
                  <button
                    key={status}
                    onClick={() => setRequestStatusFilter(status)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                      requestStatusFilter === status ? 'bg-[#FF385C] text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                  >
                    {status === 'all' ? 'All' : REQUEST_STATUS_OPTIONS.find((s) => s.value === status)?.label || status}
                  </button>
                ))}
              </div>
              <input
            ...
            {requestsLoading ? (
              <div className="flex justify-center items-center py-16">
                <div className="animate-spin rounded-full h-12 w-12 border-4 border-gray-200 border-t-[#FF385C]" />
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-1">No buy now requests</h3>
                <p className="text-gray-500 text-sm">
                  {requestStatusFilter !== 'all' || requestSearchQuery ? 'Try changing filters or search.' : 'Requests will appear when customers use "Buy now" on a product.'}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredRequests.map((req) => {
                  const statusMeta = REQUEST_STATUS_OPTIONS.find((s) => s.value === req.status) || { label: req.status }
                  const productImage = req.product?.images?.[0] || (typeof req.product?.images === 'string' ? req.product?.images : null)
                  const qty = req.quantity || 1
                  const lineTotal = req.product?.price != null ? req.product.price * qty : null
                  return (
                    <div key={req._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                req.status === 'new' ? 'bg-amber-50 text-amber-700' : req.status === 'contacted' ? 'bg-blue-50 text-blue-700' : req.status === 'completed' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'
                              }`}>
                                {statusMeta.label}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 mt-2">
                              <a href={`tel:${req.customerPhone}`} className="inline-flex items-center gap-1.5 text-sm font-medium text-[#FF385C] hover:underline">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                {req.customerPhone}
                              </a>
                              <a href={`https://wa.me/${req.customerPhone?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-medium text-green-600 hover:underline">
                            ...
                            <p className="text-sm text-gray-600 mt-2"><span className="font-medium text-gray-700">{req.city}</span><br /><span className="text-gray-500">{req.address}</span></p>
                          </div>
                          <div className="flex items-start gap-4 lg:min-w-[280px]">
                            <div className="flex-shrink-0 w-16 h-16 rounded-xl bg-gray-100 overflow-hidden">
                              {productImage ? <img src={productImage} alt={req.product?.name} className="w-full h-full object-cover" /> : (
                                <div className="w-full h-full flex items-center justify-center text-gray-400">
                                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14" /></svg>
                                </div>
                              )}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="font-medium text-gray-900 truncate">{req.product?.name || '—'}</p>
                              <p className="text-sm text-gray-500">Qty: {qty}{lineTotal != null && ` · ${formatCurrency(lineTotal)}`}</p>
                            </div>
                          </div>
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 lg:flex-shrink-0">
                            <select
                              value={req.status}
                              onChange={(e) => handleRequestStatusChange(req._id, e.target.value)}
                            ...
                            >
                              {REQUEST_STATUS_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                            </select>
                            <button
                              type="button"
                            ...
                            >
                              {deletingRequestId === req._id ? 'Deleting...' : 'Delete'}
                            </button>
                          </div>
                        </div>
                      </div>
