...
const BuyNowModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      const productId = buyNowProduct.product._id || buyNowProduct.product
  ...
  const productName = buyNowProduct.product.name || 'Product'

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div
        className={`bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[95vh] overflow-y-auto ${isRTL ? 'text-right' : 'text-left'}`}
        onClick={(e) => e.stopPropagation()}
      >
        {!thankYou ? (
          <>
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <h2 className="text-xl font-bold text-gray-900">Buy now</h2>
              <button
                type="button"
                onClick={handleClose}
                disabled={submitting}
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">Requesting: <span className="font-semibold text-gray-900">{productName}</span></p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full ${inputPad} py-3 border border-gray-200 rounded-xl focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/15 outline-none transition-all`}
                      required
                      disabled={submitting}
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className={`w-full ${inputPad} py-3 border border-gray-200 rounded-xl focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/15 outline-none transition-all resize-none`}
                      required
                      disabled={submitting}
                    />
                  </div>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-3.5 px-5 rounded-xl font-semibold bg-[#FF385C] hover:bg-[#E61E4D] text-white shadow-lg hover:shadow-xl transition-all disabled:opacity-70 flex items-center justify-center gap-2"
                >
                  {submitting ? (
                    <>
